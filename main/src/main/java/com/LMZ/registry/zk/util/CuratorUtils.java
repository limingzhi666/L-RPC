package com.LMZ.registry.zk.util;

import com.LMZ.enums.RpcConfigEnum;
import com.LMZ.utils.PropertiesUtils;
...
public final class CuratorUtils {
    ...
    // TODO 你妈的永久节点启动多个服务端会犯病要改----节点名不能固定
    public static final String ZK_REGISTER_ROOT_PATH = "/L-rpc";
    ...
    public static void createPersistentNode(CuratorFramework zkClient, String path) {
        try {
            if (REGISTERED_PATH_SET.contains(path) || zkClient.checkExists().forPath(path) != null) {
                log.info("该节点已存在。节点是:[{}]", path);
            } else {
                //eg: /my-rpc/github.javaguide.HelloService/127.0.0.1:9999
                zkClient.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(path);
                log.info("节点创建成功。节点是:[{}]", path);
            }
